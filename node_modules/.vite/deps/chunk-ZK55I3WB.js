...
} from "./chunk-X5JBBFKI.js";
import {
  Popper_default
} from "./chunk-MI47HT2V.js";
import {
...
import {
  Button_default
} from "./chunk-RVUSPGIN.js";
import {
  useLazyRef
} from "./chunk-NPW36CLJ.js";
import {
...
var selectorChartLeftAxisSize = createSelector([selectorChartRawYAxis], (yAxis) => (yAxis ?? []).reduce((acc, axis) => axis.position === "left" ? acc + (axis.width || 0) + (axis.zoom?.slider.enabled ? axis.zoom.slider.size : 0) : acc, 0));
var selectorChartRightAxisSize = createSelector([selectorChartRawYAxis], (yAxis) => (yAxis ?? []).reduce((acc, axis) => axis.position === "right" ? acc + (axis.width || 0) + (axis.zoom?.slider.enabled ? axis.zoom.slider.size : 0) : acc, 0));
var selectorChartTopAxisSize = createSelector([selectorChartRawXAxis], (xAxis) => (xAxis ?? []).reduce((acc, axis) => axis.position === "top" ? acc + (axis.height || 0) + (axis.zoom?.slider.enabled ? axis.zoom.slider.size : 0) : acc, 0));
var selectorChartBottomAxisSize = createSelector([selectorChartRawXAxis], (xAxis) => (xAxis ?? []).reduce((acc, axis) => axis.position === "bottom" ? acc + (axis.height || 0) + (axis.zoom?.slider.enabled ? axis.zoom.slider.size : 0) : acc, 0));
...
var preprocessSeries = ({
  ...
  Object.keys(seriesConfig5).forEach((type) => {
    ...
    if (group !== void 0) {
      processedSeries[type] = seriesConfig5[type]?.seriesProcessor?.(group, dataset) ?? seriesGroups[type];
    }
  });
...
var Gesture = class {
   ...
   */
  getEffectiveConfig(pointerType) {
    const baseConfig = {
  ...
  shouldPreventGesture(element, pointerType) {
    const effectiveConfig = this.getEffectiveConfig(pointerType);
...
var TapGesture = class _TapGesture extends PointerGesture {
  ...
  handlePointerEvent(pointers, event) {
    ...
    const relevantPointers = this.getRelevantPointers(pointersArray, targetElement);
    if (this.shouldPreventGesture(targetElement, event.pointerType) || relevantPointers.length < this.minPointers || relevantPointers.length > this.maxPointers) {
      if (this.isActive) {
...
function loggish(transform) {
  ...
  scale2.ticks = (count) => {
    ...
    if (!(base % 1) && j - i < n) {
      ...
      if (z.length * 2 < n) z = ticks_default(u4, v2, n);
    } else {
...
function threshold() {
  ...
  scale2.domain = function(_) {
    return arguments.length ? (domain = Array.from(_), n = Math.min(domain.length, range.length - 1), scale2) : domain.slice();
  };
  scale2.range = function(_) {
    return arguments.length ? (range = Array.from(_), n = Math.min(domain.length, range.length - 1), scale2) : range.slice();
  };
...
function calendar(ticks, tickInterval, year, month, week, day, hour, minute, second2, format2) {
  ...
  scale2.copy = function() {
    return copy(scale2, calendar(ticks, tickInterval, year, month, week, day, hour, minute, second2, format2));
  };
  return scale2;
}
function time() {
...
function transformer2() {
  ...
  scale2.range = range(value_default);
  scale2.rangeRound = range(round_default);
  scale2.unknown = function(_) {
    return arguments.length ? (unknown = _, scale2) : unknown;
  };
  return function(t) {
...
function scaleSymlog(_domain, _range) {
  const scale2 = symlog(_domain, _range);
  const originalTicks = scale2.ticks;
  const {
    negativeScale,
    linearScale,
    positiveScale
  } = generateScales(scale2);
  scale2.ticks = (count) => {
    const ticks = originalTicks(count);
    const constant = scale2.constant();
  ...
  scale2.tickFormat = (count = 10, specifier) => {
    const constant = scale2.constant();
    const [start, end] = scale2.domain();
...
function generateScales(scale2) {
  const constant = scale2.constant();
  const domain = scale2.domain();
  const negativeDomain = [domain[0], Math.min(domain[1], -constant)];
  const negativeLogScale = log(negativeDomain, scale2.range());
  const linearDomain = [Math.max(domain[0], -constant), Math.min(domain[1], constant)];
  const linearScale = linear2(linearDomain, scale2.range());
  const positiveDomain = [Math.max(domain[0], constant), domain[1]];
  const positiveLogScale = log(positiveDomain, scale2.range());
  return {
...
function createDateFormatter(data, range, tickNumber) {
  ...
  return (v2, {
    location
  }) => location === "tick" ? timeScale.tickFormat(tickNumber)(v2) : `${v2.toLocaleString()}`;
}
...
function getRange(drawingArea, axisDirection, reverse) {
...
function createAxisFilterMapper({
  ...
  return (axis, axisIndex) => {
    ...
    return createContinuousScaleGetAxisFilter(scaleType, getAxisExtremum(axis, direction, seriesConfig5, axisIndex, formattedSeries), zoom.start, zoom.end, direction, axis.data);
  };
}
...
function createContinuousScaleGetAxisFilter(scaleType, extrema, zoomStart, zoomEnd, direction, axisData) {
  let min2;
  let max2;
  [min2, max2] = getScale(scaleType ?? "linear", extrema, [0, 100]).nice().domain();
  min2 = min2 instanceof Date ? min2.getTime() : min2;
  max2 = max2 instanceof Date ? max2.getTime() : max2;
  const minVal = min2 + zoomStart * (max2 - min2) / 100;
  const maxVal = min2 + zoomEnd * (max2 - min2) / 100;
  return function filterAxis(value, dataIndex) {
    const val = value[direction] ?? axisData?.[dataIndex];
    if (val == null) {
      return true;
...
var selectorChartZoomMap = createSelector([selectorChartZoomState], (zoom) => zoom?.zoomData && createZoomMap(zoom?.zoomData));
...
var selectorChartAxis = createSelector([selectorChartXAxis, selectorChartYAxis, (_, axisId) => axisId], (xAxes, yAxes, axisId) => xAxes?.axis[axisId] ?? yAxes?.axis[axisId]);
...
function getAxisValue(axisConfig, pointerValue, dataIndex) {
  ...
  if (!isBandScale(scale2)) {
    if (dataIndex === null) {
      return scale2.invert(pointerValue);
    }
...
var selectorChartsInteractionItem = createSelector([selectInteraction], (interaction) => interaction?.item ?? null);
...
var selectorChartAxisInteraction = createSelector([selectorChartsInteractionPointerX, selectorChartsInteractionPointerY, selectorChartXAxis, selectorChartYAxis], (x2, y2, xAxis, yAxis) => [...x2 === null ? [] : xAxis.axisIds.map((axisId) => ({
  axisId,
...
function useLazySelectorEffect(store, selector, effect, skip) {
  ...
  React8.useEffect(instance.onMount(skip), [skip]);
}
...
function arc_default() {
  ...
  function arc() {
    ...
    if (!(r1 > epsilon)) context.moveTo(0, 0);
      ...
      var a01 = a0, a11 = a1, a00 = a0, a10 = a1, da0 = da2, da1 = da2, ap = padAngle.apply(this, arguments) / 2, rp = ap > epsilon && (padRadius ? +padRadius.apply(this, arguments) : sqrt2(r0 * r0 + r1 * r1)), rc = min(abs(r1 - r0) / 2, +cornerRadius.apply(this, arguments)), rc0 = rc, rc1 = rc, t03, t13;
      if (rp > epsilon) {
...
function pie_default() {
  ...
  function pie(data) {
    var i, n = (data = array_default2(data)).length, j, k2, sum3 = 0, index = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da2 = Math.min(tau, Math.max(-tau, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da2) / n, padAngle.apply(this, arguments)), pa = p * (da2 < 0 ? -1 : 1), v2;
    for (i = 0; i < n; ++i) {
    ...
    for (i = 0, k2 = sum3 ? (da2 - n * pa) / sum3 : 0; i < n; ++i, a0 = a1) {
      j = index[i], v2 = arcs[j], a1 = a0 + (v2 > 0 ? v2 * k2 : 0) + pa, arcs[j] = {
        data: data[j],
        index: i,
        value: v2,
        startAngle: a0,
...
var barValueFormatter = (v2) => v2 == null ? "" : v2.toLocaleString();
  ...
  stackingGroups.forEach((stackingGroup) => {
    ...
    ids.forEach((id, index) => {
      ...
      completedSeries[id] = _extends({
        ...
        stackedData: stackedSeries[index].map(([a2, b]) => [a2, b])
      });
    });
  });
  return {
...
var selectorChartsHighlightScopePerSeriesId = createSelector([selectSeries], (series) => {
  ...
  Object.keys(series.processedSeries).forEach((seriesType) => {
    ...
    seriesData?.seriesOrder?.forEach((seriesId) => {
      ...
      map2.set(seriesId, seriesItem?.highlightScope);
    });
  });
  return map2;
});
var selectorChartsHighlightedItem = createSelector([selectHighlight], function selectorChartsHighlightedItem2(highlight) {
  return highlight.item;
});
var selectorChartsHighlightScope = createSelector([selectorChartsHighlightScopePerSeriesId, selectorChartsHighlightedItem], function selectorChartsHighlightScope2(seriesIdToHighlightScope, highlightedItem) {
  if (!highlightedItem) {
    return null;
  }
...
var selectorChartsIsHighlighted = createSelector([selectorChartsHighlightScope, selectorChartsHighlightedItem, (_, item) => item], function selectorChartsIsHighlighted2(highlightScope, highlightedItem, item) {
  return createIsHighlighted(highlightScope, highlightedItem)(item);
});
var selectorChartsIsFaded = createSelector([selectorChartsHighlightScope, selectorChartsHighlightedItem, (_, item) => item], function selectorChartsIsFaded2(highlightScope, highlightedItem, item) {
...
var consumeThemeProps = (name, options, InComponent) => React18.forwardRef(function ConsumeThemeInternal(props, ref) {
  ...
  const OutComponent = React18.forwardRef(InComponent);
...
function ChartsTooltipContainer(inProps) {
  ...
  const anchorEl = React25.useMemo(() => ({
  ...
  const modifiers = React25.useMemo(() => [
    {
      name: "offset",
      options: {
        offset: () => {
...
true ? ChartsTooltip.propTypes = {
   ...
   * so it's simply `document.body` most of the time.
   */
  container: import_prop_types7.default.oneOfType([(props, propName) => {
    if (props[propName] == null) {
      return new Error(`Prop '${propName}' is required but wasn't specified`);
    }
    if (typeof props[propName] !== "object" || props[propName].nodeType !== 1) {
   ...
   * A ref that points to the used popper instance.
   */
  popperRef: import_prop_types7.default.oneOfType([import_prop_types7.default.func, import_prop_types7.default.shape({
    current: import_prop_types7.default.shape({
      destroy: import_prop_types7.default.func.isRequired,
      forceUpdate: import_prop_types7.default.func.isRequired,
      setOptions: import_prop_types7.default.func.isRequired,
      state: import_prop_types7.default.shape({
        attributes: import_prop_types7.default.object.isRequired,
        elements: import_prop_types7.default.object.isRequired,
        modifiersData: import_prop_types7.default.object.isRequired,
        options: import_prop_types7.default.object.isRequired,
        orderedModifiers: import_prop_types7.default.arrayOf(import_prop_types7.default.object).isRequired,
        placement: import_prop_types7.default.oneOf(["auto-end", "auto-start", "auto", "bottom-end", "bottom-start", "bottom", "left-end", "left-start", "left", "right-end", "right-start", "right", "top-end", "top-start", "top"]).isRequired,
        rects: import_prop_types7.default.object.isRequired,
        reset: import_prop_types7.default.bool.isRequired,
        scrollParents: import_prop_types7.default.object.isRequired,
        strategy: import_prop_types7.default.oneOf(["absolute", "fixed"]).isRequired,
        styles: import_prop_types7.default.object.isRequired
      }).isRequired,
      update: import_prop_types7.default.func.isRequired
    })
  })]),
  /**
   * The props used for each slot inside the Popper.
...
var consumeSlots = (name, slotPropName, options, InComponent) => {
  ...
  return React28.forwardRef(ConsumeSlotsInternal);
};
...
}, React29.forwardRef(function ChartsLegend2(props, ref) {
...
true ? ChartsLabelGradient.propTypes = {
   ...
   * @default 'horizontal'
   */
  direction: import_prop_types10.default.oneOf(["vertical", "horizontal"]),
  /**
   * A unique identifier for the gradient.
   * The `gradientId` will be used as `fill="url(#gradientId)"`.
   */
  gradientId: import_prop_types10.default.string.isRequired,
  /**
   ...
   */
  rotate: import_prop_types10.default.bool,
  /**
...
var ContinuousColorLegend = consumeThemeProps("MuiContinuousColorLegend", {
  ...
  return (0, import_jsx_runtime13.jsxs)(RootElement2, _extends({
    ...
    ownerState: props,
    children: [reverse ? maxComponent : minComponent, (0, import_jsx_runtime13.jsx)("li", {
...
true ? ContinuousColorLegend.propTypes = {
   ...
   */
  maxLabel: import_prop_types11.default.oneOfType([import_prop_types11.default.func, import_prop_types11.default.string]),
  /**
   * The label to display at the minimum side of the gradient.
   * Can either be a string, or a function.
   * @default formattedValue
   */
  minLabel: import_prop_types11.default.oneOfType([import_prop_types11.default.func, import_prop_types11.default.string]),
  /**
   ...
   * Useful for linear gradients that are not in the correct orientation.
   */
  rotateGradient: import_prop_types11.default.bool,
  /**
   * The thickness of the gradient
   * @default 12
   */
...
true ? PiecewiseColorLegend.propTypes = {
   ...
   * @default 'z'
   */
  axisDirection: import_prop_types12.default.oneOf(["x", "y", "z"]),
  /**
   * The id of the axis item with the color configuration to represent.
   * @default The first axis item.
   */
   ...
   */
  labelFormatter: import_prop_types12.default.func,
  /**
   * Where to position the labels relative to the gradient.
   * @default 'extremes'
   */
  labelPosition: import_prop_types12.default.oneOf(["start", "end", "extremes"]),
  /**
   * The type of the mark.
   * @default 'square'
   */
  markType: import_prop_types12.default.oneOf(["square", "circle", "line"]),
  /**
   * Callback fired when a legend item is clicked.
   * @param {React.MouseEvent<HTMLButtonElement, MouseEvent>} event The click event.
...
var ChartsSlotsContext = React33.createContext(null);
if (true) ChartsSlotsContext.displayName = "ChartsSlotsContext";
function useChartsSlots() {
  const context = React33.useContext(ChartsSlotsContext);
  if (context == null) {
...
true ? ChartDataProvider.propTypes = {
  ...
  apiRef: import_prop_types13.default.shape({
    current: import_prop_types13.default.any
  }),
  /**
   ...
   */
  experimentalFeatures: import_prop_types13.default.shape({
    preferStrictDomainInLineCharts: import_prop_types13.default.bool
  }),
  /**
   ...
   */
  series: import_prop_types13.default.arrayOf(import_prop_types13.default.object),
  /**
...
function ChartsWrapper(props) {
  ...
  return (0, import_jsx_runtime22.jsx)(Root3, {
    ref: chartRootRef,
    ownerState: props,
    sx,
...
function ChartsOverlay(props) {
  ...
  if (props.loading) {
    ...
    return (0, import_jsx_runtime25.jsx)(LoadingOverlay, _extends({}, props.slotProps?.loadingOverlay));
  }
  if (noData) {
    const NoDataOverlay = props.slots?.noDataOverlay ?? ChartsNoDataOverlay;
    return (0, import_jsx_runtime25.jsx)(NoDataOverlay, _extends({}, props.slotProps?.noDataOverlay));
  }
  return null;
}
...
//# sourceMappingURL=chunk-ZK55I3WB.js.map
